...
app.post("/postemotion",middleware.isAuthenticated, async (req,res)=>{  //saving the detected emotion to database to keep track of moods for mood tracker
...
app.get("/moodtracker",middleware.isAuthenticated, (req, res) => {
  ...
  app.get('/getEmotions',middleware.isAuthenticated, (req, res) => { //function to send the history of users emotion to moodTracker page
    let emotionsObj={};
    User.findById(req.user._id).populate("emotions").exec(function(err,foundUser){
